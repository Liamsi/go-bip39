...
func MnemonicToByteArray(mnemonic string) ([]byte, error) {
	...
	outLen, _ := bytelenFromWords(len(mnemonicSlice))
	toCheck := make([]byte, outLen)
	if len(entropyHex) > outLen {
		return nil, errors.New("invalid data, could not have been generated by this codec")
	}
	copy(toCheck[outLen-len(entropyHex):], entropyHex)
...
func AddChecksum(data []byte) []byte {
	...
	var res []byte
	dataFromBigInt := dataBigInt.Bytes()
	if data[0] == 0x0 {
		res = append([]byte{0x0}, dataFromBigInt...)
	} else {
		res = dataFromBigInt
	}
